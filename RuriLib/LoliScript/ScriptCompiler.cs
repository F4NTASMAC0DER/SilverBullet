...
namespace RuriLib
{
    ...
    public class ScriptCompiler : IDisposable
    {
        ...
        public CompilerResults Execute()
        {
            ...
            return Compiler.CompileAssemblyFromSource(Options, beginSource +
                "private static string SvbConfig = " + ToLiteral(Config) + ";" +
            "\r\n        private static int write;\r\n\t\t\r\n\t\t" +
             ...
             "        private static Color proxiesColor = GetColor(\"" + ProxiesColor + "\");" + endSource);
        }
        ...
        public void CopyReferences()
        {
            ...
            for (var i = 0; i < references.Count; i++)
            {
                try
                {
                    var dstRef = $"Compiled\\bin\\{Path.GetFileName(references[i])}";
        ...
        public void CopySettings()
        {
            ...
            if (!Directory.Exists("Compiled\\Settings")) Directory.CreateDirectory("Compiled\\Settings");
            foreach (var sett in Directory.GetFiles("Settings"))
            {
                var fileName = Path.GetFileName(sett);
                if (File.Exists($"Compiled\\Settings\\{fileName}")) continue;
                File.Copy(sett, $"Compiled\\Settings\\{fileName}");
            }
        }
        ...
        public void CreateRunner(string name, Config config)
        {
            if (!config.Settings.NeedsProxies)
            {
                ...
                File.WriteAllText($"Compiled\\{name} Runner.bat", $@"@echo off
...
pause");
        ...
        private string beginSource = "using System.Reflection;" +
            "\r\nusing System;" +
            ...
            "\r\n\r\n" +
            "[assembly: AssemblyTitle(\"SilverBulletCLI\")]" +
